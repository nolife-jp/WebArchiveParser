...
const config = ini.parse(
  fs.readFileSync('C:\\repos\\URLCollector\\setting.ini', 'utf-8')
);
...
(async () => {
  ...
  for (let pageNum = 0; pageNum < maxPages; pageNum++) {
    ...
    const orderableLinks = await page.evaluate(() => {
      ...
      const orderButtons = document.querySelectorAll(
        'li.list-ticket-order form.js-order_form'
      );
  ...
  if (isDiffMode) {
    ...
    const previousFiles = fs
      .readdirSync('C:\\repos\\URLCollector\\list')
      .filter((file) =>
        fs
          .statSync(path.join('C:\\repos\\URLCollector\\list', file))
          .isDirectory()
      )
      .map((dir) =>
        path.join('C:\\repos\\URLCollector\\list', dir, 'urlList.txt')
      )
    ...
    if (previousFiles.length > 0) {
      ...
      const previousLinks = fs
        .readFileSync(previousFile, 'utf-8')
        .split('\n')
        .filter(Boolean);
      const newLinks = allOrderableLinks.filter(
        (link) => !previousLinks.includes(link)
      );
